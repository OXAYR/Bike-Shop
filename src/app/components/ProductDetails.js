    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[#2D2A32]">Product not found</h1>
      ...
      <div className="container mx-auto px-4 my-8 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/2 flex flex-col-reverse md:flex-row gap-4">
                ...
                {product.images.map((image, index) => (
                  <div 
                    ...
                    className={`w-16 h-16 md:w-20 md:h-20 flex-shrink-0 cursor-pointer rounded-md border-2 overflow-hidden transition-all
                              ${selectedImage === index ? 'border-[#EAE151] shadow-md' : 'border-gray-200'}`}
            ...
            <div className="relative flex-grow h-64 sm:h-96 w-full bg-gray-50 rounded-xl overflow-hidden">
              ...
              {product.badge && (
                <div className="absolute top-4 right-4 bg-[#EAE151] text-[#2D2A32] px-3 py-1 rounded-full text-sm font-bold">
                  {product.badge}
                </div>
              )}
            </div>
          ...
          <div className="lg:w-1/2">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-[#2D2A32]">{product.name}</h1>
                ...
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <StarIcon 
                      key={i} 
                      size={16} 
                      className={i < (product.rating || 0) ? "text-[#EAE151] fill-[#EAE151]" : "text-gray-300"} 
                    />
                  ))}
                </div>
            ...
            <div className="mt-4">
              {displaySalePrice ? (
                <div className="flex items-baseline">
                  <span className="text-3xl font-bold text-[#2D2A32]">
            ...
            {product.variations && product.variations.length > 0 && (
              <div className="mt-6">
                <h3 className="text-sm font-medium text-[#2D2A32]">Options</h3>
                    ...
                    <button
                      ...
                      className={`px-4 py-2 text-sm rounded-md transition
                                ${selectedVariation?.id === variation.id 
                                  ? 'bg-[#EAE151] text-[#2D2A32] font-medium shadow-sm' 
            ...
            {/* Quantity */}
            <div className="mt-6">
              <h3 className="text-sm font-medium text-[#2D2A32]">Quantity</h3>
              <div className="flex items-center mt-2">
                <button 
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="flex-1 bg-white border-2 border-[#2D2A32] py-3 px-4 text-base font-medium text-[#2D2A32] hover:bg-gray-50 rounded-md transition flex items-center justify-center"
              ...
              <button
                ...
                className="flex-1 bg-[#EAE151] border-2 border-[#EAE151] py-3 px-4 text-base font-medium text-[#2D2A32] hover:bg-[#EAE151]/90 rounded-md transition"
                disabled={!product.inStock && (!selectedVariation || !selectedVariation.inStock)}
              >
                Buy Now
              </button>
            ...
            <div className="mt-8 border-t border-gray-200 pt-6 space-y-4">
              <div className="flex items-start">
                <Truck size={20} className="text-[#2D2A32] mr-3 mt-1" />
                <div>
                  <h4 className="font-medium text-[#2D2A32]">Free Shipping</h4>
                  <p className="text-sm text-gray-600">Free standard shipping on orders over $50</p>
                </div>
              </div>
              <div className="flex items-start">
                <RotateCcw size={20} className="text-[#2D2A32] mr-3 mt-1" />
                <div>
                  <h4 className="font-medium text-[#2D2A32]">Easy Returns</h4>
                  <p className="text-sm text-gray-600">30-day return policy</p>
                </div>
              </div>
              <div className="flex items-start">
                <Shield size={20} className="text-[#2D2A32] mr-3 mt-1" />
                <div>
                  <h4 className="font-medium text-[#2D2A32]">Warranty</h4>
            ...
            {product.specs && Object.keys(product.specs).length > 0 && (
              <div className="mt-8 border-t border-gray-200 pt-6">
                <h3 className="text-lg font-medium text-[#2D2A32]">Specifications</h3>
                <div className="mt-4 grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
                  {Object.entries(product.specs).map(([key, value]) => (
                    <div key={key} className="flex items-start">
                      <div className="flex-shrink-0">
                        <div className="h-6 w-6 rounded-full bg-[#EAE151]/20 flex items-center justify-center">
                          <Check size={14} className="text-[#2D2A32]" />
                        </div>
                      </div>
        ...
        {product.longDescription && (
          <div className="mt-16 border-t border-gray-200 pt-10">
            <h2 className="text-2xl font-bold text-[#2D2A32]">Product Description</h2>
        ...
        {product.compatibility && (
          <div className="mt-16 border-t border-gray-200 pt-10">
            <h2 className="text-2xl font-bold text-[#2D2A32]">Bike Compatibility</h2>
            <div className="mt-6 bg-gray-50 p-6 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {Object.entries(product.compatibility).map(([category, bikes]) => (
                  <div key={category}>
                    <h3 className="font-medium text-lg text-[#2D2A32] mb-2">{category}</h3>
        ...
        {product.reviews && product.reviews.length > 0 && (
          <div className="mt-16 border-t border-gray-200 pt-10">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold text-[#2D2A32]">Customer Reviews</h2>
              <button className="text-sm font-medium text-[#2D2A32] bg-[#EAE151] px-4 py-2 rounded-md hover:bg-[#EAE151]/90 transition">
                Write a Review
              </button>
            </div>
                    ...
                    <div className="flex items-center">
                      {[...Array(5)].map((_, i) => (
                        <StarIcon 
                          key={i} 
                          size={14} 
                          className={i < review.rating ? "text-[#EAE151] fill-[#EAE151]" : "text-gray-300"} 
                        />
                      ))}
                    </div>
                    <span className="text-xs text-gray-500">{review.date}</span>
                  </div>
                  <h4 className="mt-2 font-medium text-[#2D2A32]">{review.author}</h4>
        ...
        {product.relatedProducts && product.relatedProducts.length > 0 && (
          <div className="mt-16 border-t border-gray-200 pt-10">
            <h2 className="text-2xl font-bold text-[#2D2A32]">You Might Also Like</h2>
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {product.relatedProducts.map((relatedProduct) => (
                <div key={relatedProduct.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition">
                  ...
                  <div className="p-3">
                    <h3 className="font-medium text-sm text-[#2D2A32] line-clamp-2">{relatedProduct.name}</h3>
                    <div className="mt-1 flex items-center">
                      {relatedProduct.onSale ? (
                        <>
                          <span className="text-sm font-bold text-[#2D2A32]">${relatedProduct.salePrice.toFixed(2)}</span>
                          <span className="ml-1 text-xs text-gray-500 line-through">${relatedProduct.price.toFixed(2)}</span>
                        </>
                      ) : (
                        <span className="text-sm font-bold text-[#2D2A32]">${relatedProduct.price.toFixed(2)}</span>
                      )}
                    </div>
                  </div>
                </div>
